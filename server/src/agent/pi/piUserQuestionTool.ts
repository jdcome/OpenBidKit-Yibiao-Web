// Pi ask-user 自定义工具（移植自桌面 electron/services/pi/piUserQuestionTool.cjs）。
// agent 在关键事项无法从材料确定、且不同选择会实质影响结果时，暂停并向普通用户提问。
// execute 调 requestUserQuestion 挂起（由 piRuntimeService.waitForUserQuestion 承载：
// 经 EventBus 下发问题 → POST /api/agent/answer resolve）。

export const AGENT_USER_QUESTION_TOOL_NAME = 'ask-user';

import type { PiTypeBuilder } from './piJsonValidationTool';

export interface PiUserQuestionOption {
  label: string;
  description?: string;
  custom?: boolean;
}

// 工具 → runtime 的提问请求（按 tool_call_id 关联）。
export interface PiUserQuestionRequest {
  tool_call_id: string;
  question: string;
  options: PiUserQuestionOption[];
}

// runtime → 工具的作答（option_id 命中选项；custom 选项额外带 custom_answer）。
export interface PiUserQuestionResolution {
  option_id?: string;
  custom_answer?: string;
}

// requestUserQuestion 契约：piRuntimeService 实现，挂起直到 POST /answer 到达或 signal abort。
export type RequestUserQuestion = (
  request: PiUserQuestionRequest,
  signal?: AbortSignal,
) => Promise<PiUserQuestionResolution>;

export interface PiUserQuestionTool {
  name: string;
  label: string;
  description: string;
  ...
  execute: (
    toolCallId: string,
    params: { question: string; options: PiUserQuestionOption[] },
    signal?: AbortSignal,
  ) => Promise<{ content: Array<{ type: string; text: string }>; details: Record<string, unknown> }>;
}

export function createPiUserQuestionTool({
  Type,
  requestUserQuestion,
}: {
  Type: PiTypeBuilder;
  ...
  return {
    ...
    label: '询问用户',
    description:
      '当任务材料无法确定且不同选择会实质影响结果时，暂停执行并向普通用户提出一个简单易懂的问题。提供 2 至 5 个互斥选项，将推荐选项放在第一项；每个选项必须声明是否需要用户输入具体要求，且最多只能有一个自定义输入选项。',
    ...
    parameters: Type.Object(
      {
        question: Type.String({
          minLength: 1,
          description:
            '需要普通用户确认的具体业务问题，必须使用简单自然的中文，不得包含任何软件实现术语或复述任务要求；存在多个实际确认事项时使用简单 Markdown 分行列出要点。',
        }),
        options: Type.Array(
          Type.Object(
            {
