// 逐字移植自 client/electron/utils/userTextSplitter.cjs（纯函数，无 Electron/node:fs 依赖）。
// 按 LLM 上下文长度切分长文本，优先在 Markdown 标题/空行/句末/分号/逗号等自然边界断开，
// 避免切断代码围栏与代理对。knowledge_base 的 build_blocks 用它拆分超大块。
...
function normalizeContextLengthLimit(config: Record<string, unknown> | null, options: Record<string, unknown>): number {
...
function collectMarkdownFenceRanges(text: string): Array<[number, number]> {
...
function avoidsBreakingSurrogatePair(text: string, cut: number): number {
...
function clampHardCut(text: string, idealCut: number, previousCut: number, remainingSegments: number): number {
...
function canUseCandidate(candidate: number, state: ClampState): boolean {
...
export function splitUserTextByContextLimit(
  text: unknown,
