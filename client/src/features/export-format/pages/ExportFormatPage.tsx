...
function ExportFormatPage({ mode = 'create', templateId = null, onBack }: ExportFormatPageProps) {
  ...
  // admin 新建模板默认共享；编辑模式的共享状态由"我的模板"页管理（单写入口）。
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const template = currentTemplateId
        ...
        : await window.yibiao?.templates.create(nextConfig, isAdmin ? isShared : undefined);
      if (!template) {
  ...
  }, [config, currentTemplateId, showToast, isAdmin, isShared]);
  ...
  const renderLayoutSettings = () => (
    <>
      ...
      <div className="settings-list">
        ...
        {isAdmin && mode !== 'edit' ? (
          <label className="settings-row">
            <div className="settings-row-copy"><strong>设为共享模板</strong><span>共享后全员可在导出时选用</span></div>
        ...
        <div className="export-format-heading-grid">
          ...
          <label>
            ...
            <select value={cell.alignment} onChange={(event) => updateTableCell(cellKey, { alignment: event.target.value })}>
              {TABLE_CELL_ALIGNMENT_OPTIONS.map((alignment) => <option key={alignment} value={alignment}>{alignment}</option>)}
            </select>
          </label>
          <label>
            <span>文字颜色</span>
