...
import { SIZE_TO_PT, FONT_TO_CSS, ALIGNMENT_TO_CSS, PAPER_DIMENSIONS, DEFAULT_HEADING_BORDER_CELL_COLORS, TABLE_CELL_VERTICAL_CENTER_ALIGN } from '../types/exportFormat';
...
export function buildExportFormatCssVars(config: ExportFormatConfig): Record<string, string> {
  ...
  if (table) {
    ...
    tableAreas.forEach(([key, cell]) => {
      if (!cell) return;
      const isVCenter = (cell.alignment as string) === TABLE_CELL_VERTICAL_CENTER_ALIGN;
      vars[`--ef-table-${key}-font`] = chineseFontToCss(cell.font);
      vars[`--ef-table-${key}-size`] = `${chineseSizeToPt(cell.size)}pt`;
      vars[`--ef-table-${key}-align`] = alignmentToCss(isVCenter ? '居中对齐' : cell.alignment);
      vars[`--ef-table-${key}-valign`] = isVCenter ? 'middle' : 'top';
